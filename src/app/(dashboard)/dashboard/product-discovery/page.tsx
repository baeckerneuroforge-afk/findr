...
import {
  getLatestProductDiscoveryInsights,
  type ProductDiscoveryInsightRecord,
} from "@/lib/product-discovery/insights-service";
import {
...
function rollupFeatureRequestCategories(
  ...
  for (const insight of insights) {
    for (const fr of insight.result.featureRequests) {
...
function rollupPainPointCategories(
  ...
  for (const insight of insights) {
    for (const pp of insight.result.painPoints) {
...
function rollupThemes(insights: ProductDiscoveryInsightRecord[]): ThemeRollup[] {
  ...
  for (const insight of insights) {
    for (const theme of insight.result.themes) {
...
function topCalls(insights: ProductDiscoveryInsightRecord[]): CallRow[] {
  return insights
    .map((insight): CallRow => {
      const featureRequestCount = insight.result.featureRequests.length;
      const painPointCount = insight.result.painPoints.length;
      const blockerFr = insight.result.featureRequests
        .filter((fr) => fr.intensity === "blocker")
        .sort((a, b) => b.confidence - a.confidence)[0];
      const blockerPp = insight.result.painPoints
...
export default async function ProductDiscoveryOverviewPage() {
  ...
  let blockerTotal = 0;
  for (const insight of insights) {
    featureRequestTotal += insight.result.featureRequests.length;
    painPointTotal += insight.result.painPoints.length;
    for (const fr of insight.result.featureRequests) {

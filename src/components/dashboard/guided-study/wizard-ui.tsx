...
/**
 * Geteilte UI-Primitive des gefuehrten Studien-Wizards. Token-basiert
 * (bg-card / neutral-* / primary-*), also automatisch Hell/Dunkel-korrekt — im
 * Dashboard (ThemeShell .dark) rendern sie dunkel, ausserhalb hell. Keine
 * Texte hier drin: alle Beschriftungen kommen per i18n aus den Schritten.
 */
...
export function GhostButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-small font-medium text-neutral-500 outline-none transition-colors hover:text-neutral-900 focus-visible:ring-2 focus-visible:ring-primary-500/30 disabled:opacity-40"
    >
      {children}
    </button>
  );
...
export function TextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  ...
  return <input {...rest} className={`${FIELD_INPUT_CLASS} ${className}`} />;
}

export function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  const { className = "", ...rest } = props;
  return <textarea {...rest} className={`${FIELD_TEXTAREA_CLASS} ${className}`} />;
}

/* ── Auswahl-Karte (Radio-Optik) ───────────────────────────────────────── */

export function ChoiceCard({
  selected, onSelect, icon, title, desc, badge, disabled,
}: {
  selected: boolean; onSelect: () => void; icon?: React.ReactNode;
  title: string; desc?: string; badge?: string; disabled?: boolean;
}) {
  return (
    <button
      type="button"
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-3">
      <div className="min-w-0">
        <label htmlFor={id} className="block text-body-strong text-neutral-900">{title}</label>
...
export function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`rounded-xl border border-neutral-200 bg-card p-5 shadow-sm ${className}`}>{children}</div>;
}
...
export function ErrorNote({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-4 rounded-lg border border-danger-500/20 bg-danger-50 px-3 py-2 text-small text-danger-700" role="alert">
      {children}
    </p>
  );
}
